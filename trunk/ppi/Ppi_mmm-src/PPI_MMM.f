...
C
      ...
      PARAMETER (NLIST=54,MXUF=30,NHMX=50)
...
c----------------------------------------------------------------------X
C  Initialize large storage arrays DAT, TMP1, and TMP2 to BDVAL=-999.
C     LINUX - Initialize big arrays internally during execution.  
...
c----------------------------------------------------------------------X
     ...
     +          'GETLMA  ','GETSND  ','PROCESS ','STOP    '/
      ...
      DATA ITERGT,ISHADE,IEOS,IEOT,NOSCAN/1,0,0,0,0/
...
C
c      PRINT *,'LINUX - Initializing big arrays DAT, TMP1, and TMP2'
c      DO K=1,MXF
c         DO J=1,MXA
c            DO I=1,MXR
c               TMP1(I)=BDVAL
...
c         END DO
c      END DO
c      PRINT *,'LINUX - Finished initialization of big arrays'
c----------------------------------------------------------------------X
...
C
 ...
 1011 FORMAT(8X,'*********************************************',/,
     ...
     +       8X,'*              ',I6,' Fields                *',/,
     +       8X,'*              ',I6,' Functions             *',/,
     +       8X,'*              ',I6,' Contour/Swath plots   *',/,
     +       8X,'*              ',I6,' Histograms            *',/,
     +       8X,'*              ',I6,' Scatter plot lines    *',/,
     +       8X,'*-------------------------------------------*',/,
     +       8X,'*  INITIALLY USING DEFAULT FROM WORD.size   *',/,
     +       8X,'*     Add MACHSIZ command to change these   *',/,
     +       8X,'*                                           *')

      WRITE(6,1013)INT(WORDSZ)
 1013 FORMAT(8X,'* WORD SIZE = ',I2,' BITS',23X,'*')
      IF(DEC.EQ.1.0)THEN
         WRITE(6,1015)
 1015 FORMAT(8X,'* Reading input on DEC machine              *')
      ELSE
         WRITE(6,1017)
 1017 FORMAT(8X,'* Reading input on non-DEC machine          *')
      END IF
      
      IF(DECWR.EQ.1.0)THEN
         WRITE(6,1019)
 1019 FORMAT(8X,'* Input data was written on DEC machine     *')
      ELSE
         WRITE(6,1021)
 1021 FORMAT(8X,'* Input data was written on non-DEC machine *')
      END IF
      IF((DEC.EQ.1.0 .AND. DECWR.EQ.0.0).OR.
     +   (DEC.EQ.0.0 .AND. DECWR.EQ.1.0))THEN
         WRITE(6,1023)
 1023 FORMAT(8X,'*    **** BYTE-SWAPPING IS REQUIRED ****    *')
      END IF
...
c      print *,'  igo=',indat(1),igo
 ...
 9       FORMAT(1X,'INVALID COMMAND: ',10A8)
      ...
      IF(INDAT(1)(1:4).EQ.'STOP')THEN
         ISTOP=1
         WRITE(6,91)NFRAME
 91      FORMAT(8X,' STOP PROCESSING:    FRAMES PLOTTED=',I6)
...
C             CONVERT PREVIOUS SWEEP ENDING TIME (ITOLD - HHMMSS)
      ...
      IF(NSWTH.GE.1)THEN
         ...
         END IF
      END IF
      print *,'PPI_MMM: mscan,mscanold,nswpavg,jmod=',
     +     mscan,mscanold,nswpavg,jmod
      print *,'PPI_MMM: itim1,iftim,itim,itim2=',
     +     itime1,iftime,itime,itime2
      print *,'PPI_MMM:    fxang1,fxang,fxang2=',fxang1,fxang,fxang2
C
...
C
      ...
      IF(NFXVOL.LE.NFXMAX)THEN
         FXVOL(NFXVOL)=FXOLD
      END IF
      print *,'Before CALL FUNC: nfxvol,fxvol=',nfxvol,fxvol(nfxvol)
...
c-----print *,'aftfun:mscan,fxold,iw,nw=',mscan,fxold,iwin-1,nwin
...
C        2) When any one of PLTPROJ, PLTAE, PLTAHST, or PLTASCT requested.
...
c-----debug (ljm)
      if(nswth.ge.1)then
         print *,'swath=',jndat(1),isctp(itpold),scantyp,
     +        nswpavg,pltlast,nwin,iwin
      end if
c-----debug (ljm)
...
c-----debug (ljm)
      if(nswth.ge.1)then
         print *,'CntSw: nswth,mscan,plteov,pltlast=',
     +        nswth,mscan,plteov,pltlast
         write(6,1770)itimbov,itimeov,fxold,nshort,nbvol,nfxvol,
     +        nfxhst,nfxsct,itpold,itp,ivolold,ivol,ieof,ieot,istop,
     +        mscan,nswpavg,mod(mscan,nswpavg),isctp(itpold),scantyp
1770    format('*cntsw:time,fx,flg=',i6.6,'-',i6.6,f6.1,13i4,i7,i4,
     +        2a4)
      end if
c-----debug (ljm)
...
c               print *,'pltproj: fr,i,iw,nw=',nframe+1,i,iwin-1,nwin
     ...
     X              JMRK)
               ...
               YMN=GYMIN(JTP)
               YMX=GYMAX(JTP)
c               write(6,1772)jtp,xmn,xmx,ymn,ymx,iftime,itime
c 1772          format('pltact:',i8,4f8.1,2i8)
...
C
     ...
     X               (NFXHST.EQ.NSWPAVG .OR. NSWPAVG.EQ.999999))THEN
                     AHSTCLR=.TRUE.
                  END IF
                  print *,'pltahst: fr,i,iw,nw=',nframe+1,i,iwin-1,nwin
                  print *,'PLTAHST: clr,nfx,navg=',ahstclr,nfxhst,
     +                 nswpavg
                  CALL PLTHIST(NAH,IHNAM,HRMN,HRMX,HAMN,HAMX,
     X                 HZMN,HZMX,X0,Y0,H0,IHKP,JHKP,FMN,FMX,FBIN,FREF,
     X                 PMN,PMX,HTYP,HFIT,PLTSW,NFRAME,AHST,AHSTCLR,
...
c                  print *,'pltasct: fr,i,iw,nw=',nframe+1,i,iwin-1,nwin
                  print *,'PLTASCT: clr,nfx,navg=',asctclr,nfxsct,
     +                 nswpavg
...
C
  ...
  851    FORMAT(8X,' END OF DATA ON UNIT: ',I2,
     +             ' FRAMES PLOTTED=',I6)
         GO TO 5
      END IF
 ...
 1773    FORMAT(1X,' IDATE,ITIME,NEWDAY,IETIME=',I8,2X,I6.6,I8,2X,I6.6)
         WRITE(6,853)NFRAME
  853    FORMAT(8X,'     END PROCESS: FRAMES PLOTTED =',I6)
         GO TO 5
      ELSE
         GO TO 810
      END IF
