...
c----------------------------------------------------------------------X
c
      SUBROUTINE GETSND(INDAT,OLAT,OLON,ANGXAX,ORLAT,ORLON,NFRAME)
C
C  Read in mobile GLASS sounding from SNDFILE
...
C
      ...
      CHARACTER SNDFILE*24,IDIR*4,SDFILT*8,TTYP*3,CTYP*3
      CHARACTER*8 INDAT(10),JNDAT(10),BLANK
      character*80 mglass_line,title
      character*1 pltskewt
      character proj*4,site*12,lat*10,lon*11,year*4,mon*2,day*2,tyme*6

      DATA BLANK,BADVAL/'        ',-999.9/
      DATA TODEG,ABSC/57.29577951,273.16/

      READ(INDAT,1)TTYP,CTYP,IDIR,OLAT,OLON,SDFILT,RTFILT,PLTSKEWT
...
c      TAIR=TDRY(AOD,BASEP) - ABSC
      ...
      WRITE(6,51)BASEP,BASET,BASEZ,AOS,MIXBASE,AOD
 51   FORMAT(1X,'CLOUD BASE CONDITIONS: P=',F6.1,' T=',F6.1,' Z=',F6.1,/,
     +     23X,'ThetaE=',F6.1, ' MixRatio=',F6.1,' Theta=',F6.1)

C     Read initial lines of header information
C     This code may need to be modified some, depending on the
C     way lines within the sounding file are laid out.
C
      do i=1,15
         read(9,7)mglass_line
 7       format(a80)
         print *,mglass_line
         if(i.eq.2)write(proj,71)mglass_line(36:39)
         if(i.eq.3)write(site,72)mglass_line(36:47)
         if(i.eq.4)write(lon, 73)mglass_line(36:46)
         if(i.eq.4)write(lat, 74)mglass_line(49:58)
         if(i.eq.5)write(year,75)mglass_line(36:39)
         if(i.eq.5)write(mon, 76)mglass_line(41:42)
         if(i.eq.5)write(day, 77)mglass_line(43:44)
         if(i.eq.5)write(tyme,78)mglass_line(46:51)
 71      format(a4)
 72      format(a12)
 73      format(a11)
 74      format(a10)
 ...
 78      format(a6)
      ...
      write(title,79)proj,site,lat,lon,year,mon,day,tyme
 79   format(a4,'-',a12,' [',a10,' ',a11,'] ',a4,' ',a2,a2,'-',a6)
      print *,title
c      write(6,8)
c      write(6,9)
...
c     +     '   m/s     deg     deg     km   deg     m')
      ...
      ISND=0
 10   CONTINUE
      READ(9,*,ERR=10,END=20)TIME,PRESS,TEMP,DEWPT,RH,U,V,SPD,DIR,
...
c 11   format(f6.1,f7.1,3f6.1,2f7.1,3f6.1,f9.3,f8.3,2f6.1,f8.1)
      IF(TIME.LT.0.0)GO TO 10
      IF(PRESS .GE. 999.0)GO TO 10
      IF(TEMP  .GE. 999.0)GO TO 10
      IF(DEWPT .GE. 999.0)GO TO 10
      IF(RH    .GE. 999.0)GO TO 10
      if(U     .GE. 999.0)GO TO 10
      if(V     .GE. 999.0)GO TO 10
      if(SPD   .GE. 999.0)GO TO 10
      if(DIR   .GE. 999.0)GO TO 10
      if(wascent.ge.999.0)wascent=badval
...
C     Compute the average pressure increment and set 
C     skipping factor (IBSKIP) for wind barbs.
C
      delpres=abs(psnd(isnd)-psnd(1))/float(isnd)
      ibskip=nint(20.0/delpres)
      print *,'IBSKIP=',ibskip
      if(ibskip.le.0)ibskip=1
      print *,'Reset IBSKIP=',ibskip

      BDVAL=BADVAL
      IF(SDFILT.NE.'NONE')THEN
         IRAD=INT(RTFILT)
         CALL SMTH1D(USND,SDFILT,IRAD,BDVAL,1,ISND,ISND)
         CALL SMTH1D(VSND,SDFILT,IRAD,BDVAL,1,ISND,ISND)
         CALL SMTH1D(WSND,SDFILT,IRAD,BDVAL,1,ISND,ISND)
         CALL SMTH1D(PSND,SDFILT,IRAD,BDVAL,1,ISND,ISND)
         CALL SMTH1D(TSND,SDFILT,IRAD,BDVAL,1,ISND,ISND)
         CALL SMTH1D(DPSND,SDFILT,IRAD,BDVAL,1,ISND,ISND)
         CALL SMTH1D(RHSND,SDFILT,IRAD,BDVAL,1,ISND,ISND)
      END IF
...
C
      IF(PLTSKEWT.EQ.'Y')THEN
         CALL DRV_SKEW(PSND,TSND,DPSND,SSND,DSND,MXSND,ISND,TITLE,
     X        AOS,AOD,BASEP,BASET,MIXBASE,IBSKIP,BADVAL)
         CALL FRAME
         NFRAME=NFRAME+1
      END IF
...
c----------------------------------------------------------------------X
c
      subroutine drv_skew(psnd,tsnd,dpsnd,ssnd,dsnd,mxsnd,
     x     isnd,title,aos,aod,basep,baset,mixbase,ibskip,
     x     badval)
c
...
c
      ...
      DIMENSION SSND(MXSND),DSND(MXSND)
      parameter (mm=5000)
      dimension pres(mm),temp(mm),dwpt(mm),spd(mm),dir(mm)
      dimension tcld(mm),delt(mm)
...
c            tkelvin = temp(i)+absc
         ...
         write(6,15)i,pres(i),temp(i),dwpt(i),dir(i),spd(i),
     x        tcld(i),delt(i),mixrat(i),lwc(i)
 15      format('i=',i5,' p,T,Td,Dir,Spd=',5f7.1, 
...
c     
      print *,'Calling skewt; nn=',nn
      call skewt (pres,temp,dwpt,tcld,spd,dir,nn,title,badval,ibskip)
C                                                                       
      print *,'Back from skewt'
c
c     ... shut down GKS
c
...
c----------------------------------------------------------------------X
c
      SUBROUTINE SKEWT (PRES,TEMP,DWPT,TCLD,SPD,DIR,NN,TITLINE,BADVAL,
     X     IBSKIP)
C                                                                       
...
C                                                                       
      DIMENSION PRES(1),TEMP(1),DWPT(1),TCLD(1),SPD(1),DIR(1)                   
...
C                                                                       
      ...
      DATA ISWIDE / 1200 /                                              
C
C  SKIPPING FACTOR FOR WINDBARBS
C
c      DATA IBSKIP/50/
...
C     Go to 61 to terminate at first BADVAL
C
      CALL SETUSV('LW',3000)                                      
      DO 60 I=1,NN                                                      
        IF(TEMP(I).EQ.BADVAL)GO TO 60
          Y=FY(PRES(I))                                                 
          X=FX(TEMP(I),Y)                                               
          IF (I.EQ.1) CALL FRSTPT(X,Y)                                  
          CALL VECTOR(X,Y)
   60 CONTINUE                                                          
   61 CONTINUE                                                          
      print *,'Finished p,T'
...
C     Go to 71 to terminate at first BADVAL
C
      CALL SETUSV('LW',1500)                                      
      DO 70 I=1,NN                                                      
         IF(DWPT(I).EQ.BADVAL)GO TO 70                                  
            Y=FY(PRES(I))                                               
            X=FX(DWPT(I),Y)                                             
            IF(I.EQ.1)CALL FRSTPT(X,Y)                                  
            CALL VECTOR(X,Y)                                            
...
C
      ...
      JFLAG=0
      DO 80 I=1,NN-1
         IF(TCLD(I).EQ.BADVAL)GO TO 80
         IF(TCLD(I+1).EQ.BADVAL)GO TO 80
            Y1=FY(PRES(I))                                               
            X1=FX(TCLD(I),Y1)                                             
            Y2=FY(PRES(I+1))                                               
            X2=FX(TCLD(I+1),Y2)                                             
...
C                                                                       
      CALL SETUSV (ISPOPT,ISWIDE)                                       
      DO 75 I=1,NN,IBSKIP
         IF (DIR(I) .EQ. BADVAL)GO TO 75
         IF (SPD(I) .EQ. BADVAL)GO TO 75
