...
c----------------------------------------------------------------------X
c
      SUBROUTINE FLDIDDO(NAMFLD,NFLDS,FLDNAM,NFLD,IFLD)
C
C  DETERMINE IF REQUESTED FIELDS ARE ON THE INPUT DORADE FORMAT TAPE
C
C  INPUT:
C        NAMFLD - NAME OF FIELD REQUESTED
C        NFLDS  - TOTAL NUMBER OF FIELDS REQUESTED
C        FLDNAM - NAME OF FIELDS ON THE DORADE FORMAT TAPE
C        NFLD   - NUMBER OF FIELDS ON THE DORADE FORMAT TAPE
C  OUTPUT:
...
C
      ...
      CHARACTER*8 NAMFLD(NFLDS),FLDNAM(NFLD)
   ...
   11 FORMAT(1X,/,' Fields to be processed = ',I3,
     ...
     +       /,1x,'       (-6)   "    " is the number averaged'/)
      ...
      DO 100 I=1,NFLDS
         ...
         IFLD(I)=0
         DO 80 J=1,NFLD
   80    IF(NAMFLD(I).EQ.FLDNAM(J))IFLD(I)=J
   90    CONTINUE
