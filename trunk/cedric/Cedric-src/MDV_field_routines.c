...
#include "./include/toolsa_macros.h"
#include <math.h>
...

{
  ...
  /*printf("in fhand read vol %d\n",volume_size);*/
  if (fread(buf, 1, volume_size, infile)
      != volume_size) {
...

{
  ...
  if (!MDV_compressed(handle->hdr.compression_type)) {
    ...
    if (fread(buf, 1, input_len, infile) != input_len) {
    ...
    if (fread(&be_input_offset, sizeof(ui32), 1, infile) != 1) {
      fprintf(stderr, "ERROR - MDV_fhand_read_plane\n");
    ...
    if (fread(&be_input_len, sizeof(ui32), 1, infile) != 1) {
      fprintf(stderr, "ERROR - MDV_fhand_read_plane\n");
...

{
  ...
  for (iz = 0; iz < nz; iz++) {
    ...
    if(compression_type ==  MDV_COMPRESSION_RLE){
      ...
      fprintf(stderr, " cedric is unable to handle compression type: %d\n", compression_type);
      MEMbufDelete(work_buf);
      exit(0);
    }
