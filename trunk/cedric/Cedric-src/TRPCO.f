...
c-------*-------*-------*-------*-------*-------*-------*-------*-------*
      DIMENSION FLD(MAXX,MAXY,MAXZLEV)
      LOGICAL ARRAY
      DATA ARRAY /.TRUE./
c-------*-------*-------*-------*-------*-------*-------*-------*-------*
...
c-------*-------*-------*-------*-------*-------*-------*-------*-------*
         if(array)then
c
c           Fetch a field for all z-levels - does this only once
c           Field values are then moved rather than fetch'd.
c
            do kk=1,ncx(3)
               call clock(tyme)
 ...
 1773          format('trpco-call fetchd for plane   kk=',i4,
     ...
     x              bad,zlev,nst)
               if (nst.ne.0) then
                  write(*,*)'+++error: trpco - fetchd into array+++'
                  call flush_stdout
               end if
               do jj=1,ncx(2)
                  do ii=1,ncx(1)
...
c-------*-------*-------*-------*-------*-------*-------*-------*-------*
                        if(array)then
c
c                       Move field values rather than fetching them
c
                           do jj=1,n2
                              do ii=1,n1
...
c
c                       Fetch field values
c
                           CALL FETCHD(IBOT,ID,N,I,IBUF,RBOT,N1,N2,3,
     X                          BAD,ZLEV,NST)
                           IF (NST.NE.0) THEN
                              WRITE(*,*)'+++ERROR: TRPCO - FETCHD #1+++'
                              CALL FLUSH_STDOUT
                           END IF
                        end if
c-------*-------*-------*-------*-------*-------*-------*-------*-------*
...
c
     ...
     X                          BAD,ZLEV,NST)
                           IF (NST.NE.0) THEN
                              WRITE(*,*)'+++ERROR: TRPCO FETCHD #2+++'
                              CALL FLUSH_STDOUT
                           END IF
                           CALL FETCHD(ITOP,ID,N+1,I,IBUF,RTOP,N1,N2,
     X                          3,BAD,ZLEV,NST)
                           IF (NST.NE.0) THEN
                              WRITE(*,*)'+++ERROR: TRPCO FETCHD #3+++'
                              CALL FLUSH_STDOUT
                           END IF
                        end if
c-------*-------*-------*-------*-------*-------*-------*-------*-------*
...
c                          Fetch field values
c
                           CALL FETCHD(ITOP,ID,N+1,I,IBUF,RTOP,N1,N2,3,
     X                          BAD,ZLEV,NST)
                        end if
c-------*-------*-------*-------*-------*-------*-------*-------*-------*
                     END IF
