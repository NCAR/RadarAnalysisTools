...
void nexrad_rdbeam(int *inunit, int *irewnd, int *istat,int *swapping,
                    char tape_unit[8],char fldnam[MXFLD][8],
                    float flddat[MXFLD][MXGAT],float gatspac[MXFLD],
                    float rng_first_gate[MXFLD],int nrng[MXFLD],
                    int *ivol,int *iswp, int *iyr, int *mon, int *day,
                    int *hr, int *min, int *sec, int *msec, int *julday,
                    float *az, float *el,  float *fixed_angle,
                    float *nyquist,int  *nfields,int *num_rays,int *printi)
...
#elif defined (LINUX)
void nexrad_rdbeam__(int *inunit, int *irewnd, int *istat,int *swapping,
                    char tape_unit[8],char fldnam[MXFLD][8],
                    float flddat[MXFLD][MXGAT],float gatspac[MXFLD],
                    float rng_first_gate[MXFLD],int nrng[MXFLD],
                    int *ivol,int *iswp, int *iyr, int *mon, int *day,
...
#else
void nexrad_rdbeam_(int *inunit, int *irewnd, int *istat,int *swapping,
                    char tape_unit[8],char fldnam[MXFLD][8],
                    float flddat[MXFLD][MXGAT],float gatspac[MXFLD],
                    float rng_first_gate[MXFLD],int nrng[MXFLD],
                    int *ivol,int *iswp, int *iyr, int *mon, int *day,
   ...
   */
   for(ii = 0; ii < MXFLD; ii++){
     for(jj = 0; jj < MXGAT; jj++){
         flddat[ii][jj] = -32768.0;
     }
   } 

   /*
    *Initialize the field name arrays.
    */
   for(ii = 0; ii < MXFLD; ii++){
     for(jj = 0; jj < 8; jj++){
         cfldnam[ii][jj] = ' ';
   ...
   read_status = read_nexrad_ray(flddat,cfldnam,gatspac,nrng,rng_first_gate,swap,
                                 &number_of_rays);
   switch(read_status){
  ...
  case DATA_RECORD:{ /*A message header has ray time information in it*/
	 /*
          ...
          */
          ...
          if(drdh->radial_status == 3){
             vol = vol + 1;
             sweep_num = 1;
	     /*printf("START OF A NEW VOLUME\n");*/
	  }
          ...
          *ivol    = vol;
          ...
          if(number_of_rays == 1){
             ...
             if(*printi == 1){
                printf("\nSTART OF NEW SWEEP\n");
                printf("VOLUME COVERAGE PATTERN FOR THIS SWEEP = %d\n",current_VCP);
                printf("SWEEP NUMBER %d IN VOLUME %d\n",sweep_num,vol);
                printf("FIELD   GATES    FIRST  SPACING   NYQUIST\n");
                if(cfldnam[0][0] == 'D'){
                    printf("  %c%c    %3d    %.2f   %.2f\n",cfldnam[0][0],
                           cfldnam[0][1],nrng[0],gatspac[0],rng_first_gate[0]);
	        }
                if(cfldnam[1][0] == 'V'){
                    printf("  %c%c    %3d    %.2f   %.2f   %.2f\n",cfldnam[1][0],
                           cfldnam[1][1],nrng[1],gatspac[1],rng_first_gate[1],
                           (float)drdh->nyquist_vel_x100/100.);
                    printf("  %c%c    %3d    %.2f   %.2f\n",cfldnam[2][0],
                           cfldnam[2][1],nrng[2],gatspac[2],rng_first_gate[2]);
	        }
	      }/*printi*/

             sweep_num = sweep_num + 1;
	  }/*if number of rays is 1*/
          ...
          *nyquist     = (float)drdh->nyquist_vel_x100/100.;
          ...
          if(*printi == 1){
              if(*num_rays == 1) printf("\n  AZ     ELEV    FIXED       TIME       RAYS  ISTAT\n");
              printf("%.2f   %.2f    %.2f ",*az,*el,*fixed_angle);
              printf("  %2d:%2d:%2d:%3d   %3d      %d\n",*hr,*min,*sec,*msec,number_of_rays,
                        drdh->radial_status);
	  }
...
extern struct digital_radar_data_header *drdh;
	 ...
	 /*printf("tape header\n");*/
         return(TAPE_HEADER);
           ...
           init_gate_info(nrng,gatspac,rng_first_gate);
           position  = position + nmh_size;
           if(blockType == DIGITAL_RADAR_DATA){
               *number_of_rays = *number_of_rays + 1;
               get_digital_radar_data_header(ndi->fileBuffer,position,cfldnam,swap);
/*NOTE: ACCORDING TO THE NEXRAD LEVEL 2 DOCUMENTATION ANY BASE DATA THAT HAS A VALUE OF 0
 *IS DATA BELOW SIGNAL TO NOISE RAIO.  ANY BASE DATA VALUE OF 1 IS DATA CONSIDERED RANGE
 *AMBIGUOUS.  SO VALUES OF 1 AND 0 ARE SET TO THE BAD DATA FLAG.
 */
               if(cfldnam[0][0] == 'D'){
                           nrng[0]           = drdh->ref_num_gates;
                           start_of_data     = drdh->ref_ptr;
                           gatspac[0]        = drdh->ref_gate_width;
                           rng_first_gate[0] = drdh->ref_gate1;
                           offset = 32.;
                           ...
                           */
                           ...
                           for(index = 0; index < number_of_gates; index++){
                               ...
                               if((unsignedByte != 0) && (unsignedByte != 1)){
                                   ...
                                   flddat[0][index] = (float)databuff[index];
		               }
		            }/*for loop for data*/
                            free(databuff);
		}
                if(cfldnam[1][0] == 'V'){
                           nrng[1]           = drdh->vel_num_gates;
                           start_of_data     = drdh->vel_ptr;
                           gatspac[1]        = drdh->vel_gate_width;
                           rng_first_gate[1] = drdh->vel_gate1;
                           ...
                           */
                           ...
                           for(index = 0; index < number_of_gates; index++){
                               ...
                               if((unsignedByte != 0) && (unsignedByte != 1)){
                                   databuff[index] = (double)((unsignedByte - 2) * 0.5) - offset;
                                   flddat[1][index] = (float)databuff[index];
		               }
		            }/*for loop for data*/
                            free(databuff);
                 ...
                 if(cfldnam[2][0] == 'S'){
                            nrng[2]           = drdh->vel_num_gates;
                            start_of_data     = drdh->sw_ptr; 
                            gatspac[2]        = drdh->vel_gate_width;
                            rng_first_gate[2] = drdh->vel_gate1;
                            offset = 63.5;
                           /*
                           *Malloc space for the data buffer.
                           */
                           ...
                           for(index = 0; index < number_of_gates; index++){
                               ...
                               if((unsignedByte != 0) && (unsignedByte != 1)){
                                   databuff[index] = (double)((unsignedByte - 2) * 0.5) - offset;
                                   flddat[2][index] = (float)databuff[index];
		               }
		            }/*for loop for data*/
                            free(databuff);
                  }
                    ...
                    float rng_first_gate[MXFLD])
{
   ...
   for(index = 0; index < MXFLD; index++){
       nrng[index] = 0;
       gatspac[index] = -32768.0;
       rng_first_gate[index] = -32768.0;
   }
