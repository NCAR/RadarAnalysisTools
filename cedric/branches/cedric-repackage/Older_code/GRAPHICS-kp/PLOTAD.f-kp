...
C
 ...
 103     FORMAT(A1,' ',A4)
      ...
      IF (LABFLG.GT.5) THEN
         CALL PLCHMQ(CPUX(10),CPUY(LOCY),LABEL(1:16),12.,90.,-1.)
      END IF
      WRITE (LABEL,103)AXNAM(L1), LABAXS(L1,IUNAXS)
      LOCY=LOCPLT(YB)-50
      IF (LABFLG.GT.5) THEN
         CALL PLCHMQ(CPUX(150),CPUY(LOCY),LABEL(1:6),12.,0.,-1.)
      END IF
   10 CONTINUE
C
...
C
  ...
  108 FORMAT('EVERY ',I2,' POINT IS PLOTTED')
      IF(IOLAY.EQ.2) THEN
         IF (LABFLG.GT.5) THEN
            CALL PLCHMQ(CPUX(125),CPUY(880),LABEL(1:25),12.,0.,-1.)
            WRITE (LABEL,105)(NAMF(I),I=1,4)
 105        FORMAT('OVERLAY FIELD IS ',4A2)
            CALL PLCHMQ(CPUX(500),CPUY(880),LABEL(1:25),12.,0.,-1.)
         END IF
      ELSE
         IF (LABFLG.GT.5) THEN
            CALL PLCHMQ(CPUX(275),CPUY(LOCY),LABEL(1:25),12.,0.,-1.)
         END IF
      END IF
  ...
  107 FORMAT('( X ',F7.2,' )')
      IF (LABFLG.GT.5) THEN
         CALL PLCHMQ(CPUX(230),CPUY(985),LABEL(1:13),12.,0.,-1.)
      END IF
C
...
C
   ...
   15    CONTINUE
            WRITE (LABEL,104)X
  104       FORMAT(F6.1)
            CALL PLCHMQ(CPUX(LOCX),CPUY(LOCY),LABEL(1:6),8.,0.,-1.)
            X=X+WIDTH
            LOCY=LOCY-17
...
C
            ...
            LOCY=934
            KHAR=ISYM(NDSYM,1)
            CALL PLCHMQ(CPUX(LOCX),CPUY(LOCY),KHAR(1:1),8.,0.,0.)
            DO 25 K=1,2
               LAST=NSYM(K)
               ...
               DO 20 I=1,LAST
                  ...
                  LOCY=LOCY-17
                  KHAR=ISYM(J,K)
                  CALL PLCHMQ(CPUX(LOCX),CPUY(LOCY),KHAR(1:1),
     X                 8.,0.,0.)
   20          CONTINUE
   25       CONTINUE
            LOCY=LOCY-17
