      SUBROUTINE REMAP(KRD,IBUF,RBUF,LCMB2,MAXPLN,IPR,NST,IFLAT,
...
C        which can be ('CRT','COPL', or 'ELEV').

C     Input coordinate system is specified by ICORD.
C        (0) Cartesian - levels of constant msl height z (x,y,z)
C        (1) Coplane -   levels of constant coplane angle c (xc,yc,c)
C        (2) Elevation - levels of constant elevation angle e (x,y,e)
...
C     STATION LOCATIONS
      ...
      PARAMETER (MAXZLEV=128)
      COMMON /LEVELS/ VALLEV(MAXZLEV)
      CHARACTER IXOR*12
      CHARACTER*1 CIBL
      CHARACTER*2 CTEMP
      CHARACTER*8 CTEMP1,CTEMP2,NAMIN(3),NAMOUT(3)
...
C     
      READ(KRD,100)CTEMP1,ANGXAX,XORG,YORG,ZORG,CTEMP2,RELMAX
 100  FORMAT(/A8/F8.2/F8.2/F8.2/F8.2/A8/F8.2)

      IF (CTEMP1.EQ.'COPLANE') THEN
C     
C     COPLANE -> CART (ICORD = 1)
C     
         WRITE(CTEMP1,110)ID(16)
 110     FORMAT(A2)
         IF (CTEMP1.NE.'CO') THEN
            CALL CEDERX(575,1)
            RETURN
         END IF
         ICORD=1
      ELSE IF (CTEMP1.EQ.'CART') THEN
C     
C     CART ->    CART (ICORD = 0)
C     
         ICORD=0
...
C     
 ...
 202           FORMAT(5X,'+++  NUMBER OF POINTS ALONG THE  ',A2,
     X              'AXIS MUST BE IN THE RANGE OF  1 TO ',I3,'  +++')
            ...
            END IF
 30      CONTINUE
         IF ((NCXN(1)*NCXN(2)*NCXN(3)*NF).GE.8000000) THEN
            WRITE(*,33)
...
C     
 ...
 104  FORMAT(//3X,'REMAPPING SPECIFICATIONS FOR THE NEW CARTESIAN',
     X     ' COORDINATE SYSTEM ...'/
     X     8X,'1-  X-AXIS ORIENTATION REL. TO NORTH:',3X,A12/
     X     8X,'2-  (X,Y,Z) POSITION OF THE NEW ORIGIN: (',F7.2,',',
     X     F7.2,',',F7.2,')'/
...
C     
 ...
 525              FORMAT(5X,'NYQUIST VELOCITY=',F8.2)
               ELSE
C
C     USE USER SUPPLIED NYQ. VELOCITY
C
...
C
         IF ((ICORD.EQ.1) .AND. (NAMIN(1).EQ.' ' .OR. NAMIN(2).EQ.' ' 
...
C
 ...
 170  CONTINUE
     ...
     X           ITEMP,IBUF,IDIM2,MEMUSE)
         ELSE
C     
C     INTEGRAL SHIFT ONLY
C     
...
C
      ...
      IF (ICORD.EQ.1) THEN
C
C     CHANGE AXIS LABELS BECAUSE DATA IS NOW IN CARTESIAN SPACE
C
         CTEMP='KM'
