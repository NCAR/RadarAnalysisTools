...
C      print *,'PLTACT-2: labflg=',labflg
      IF (LABFLG.GT.5)THEN
         CALL MY_PLCHMQ(220,985,SYMBS2(IMAJPAT),SYMSIZ2,0.,-1.)
         WRITE(JTIT,200)INT(TMAJOR)
 200     FORMAT(' EVERY ' ,I4,' SEC')
         CALL MY_PLCHMQ(227,985,JTIT(1:15),12.,0.,-1.)
      END IF
      CALL SFLUSH
      CALL GSPLCI(IOLDCOL)
      CALL GFLAS2
...
C     
 ...
 500     FORMAT(5A8,39A1,'X')
         print *,'PLTACT: itit =',itit
         print *,'PLTACT: citit=',citit
         CALL DATEE(NOW)
         CF=1./ID(69)
         XOR=ID(40)*CF
         SF=1./ID(68)
