...
c----------------------------------------------------------------------X
c
      SUBROUTINE SMTH1D(ADAT,IFLTYP,IRAD,BDVAL,IMN,IMX,MXA)
C
...
C
      ...
      CHARACTER*8 IFLTYP
      CHARACTER*8 NFILT(5)
      DIMENSION ADAT(MXA),FDAT(MXT)
      DATA NFILT/'UNIF','TRIA','CRES','QUAD','EXPO'/
      DATA CNTMN,EPS/3.0,1.0E-6/

      print *,'SMTH: ifltyp,irad,bdval=',ifltyp,irad,bdval
      print *,'SMTH:       imn,imx,mxa=',imn,imx,mxa

      INDX=PPI_IFIND(IFLTYP,NFILT,5)
      BIGR=(IRAD+1)**2
      IF(INDX.EQ.0)THEN
         WRITE(6,5)
    5    FORMAT(1X,'*** WARNING - UNKNOWN FILTER, RESET TO TRI ***')
      ...
      IB=IMN+IRAD
      IE=IMX-IRAD
      print *,'SMTH:   indx,bigr,ib,ie=',indx,bigr,ib,ie
      DO 50 I=IB,IE
         FDAT(I)=BDVAL
         CNT=0.0
         SUMW=0.0
