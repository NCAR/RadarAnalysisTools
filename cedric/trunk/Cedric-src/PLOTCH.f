...
c      PARAMETER (NRWRK=7000,NIWRK=2000,NAWRK=50000,NIARA=20)
      ...
      DIMENSION ID(NID),IAXIS(3)
      CHARACTER*16 CFMTX,IFMTX,IFMTY
      CHARACTER*16 CFMTY
      CHARACTER*8 IPAT(3),NOWDAT,ITIT(5)
      CHARACTER*80 LABEL
      CHARACTER*40 CITIT
      CHARACTER*2 NAMF(4)
      CHARACTER*1 IPAX
...
C     
 ...
 103     FORMAT(A3,' ',A4)
      ...
      IF (LABFLG.GT.5) THEN
         CALL PLCHMQ(CPUX(10),CPUY(LOCY),LABEL(1:7),12.,90.,-1.)
         WRITE (LABEL,103)AXNAM(L1), LABAXS(L1,IUNAXS)
         LOCY=LOCPLT(YB)-50
         CALL PLCHMQ(CPUX(150),CPUY(LOCY),LABEL(1:7),12.,0.,-1.)
      END IF
    5 CONTINUE
      IF(IOLAY.EQ.2 .AND. LABFLG.GT.5) THEN
         WRITE (LABEL,105)(NAMF(I),I=1,4)
 105     FORMAT('OVERLAY FIELD IS ',4A2)
         CALL PLCHMQ(CPUX(500),CPUY(900),LABEL(1:25),12.,0.,-1.)
      END IF
...
C     
     ...
     X     CABDBZ,IOFFP,BAD,NPATC,IPLTYP,RMAX,RMIN,CPFX(LOCX),
     X     CPFY(LOCY),YB,WBUF,IGREY,THKLIN,LABFLG)
...
C
      IF (LABFLG.GT.5) THEN
         ...
         CALL GSCLIP(0)
         CALL PLCHMQ(CPUX(LOCX),CPUY(LOCY-40),LABEL(1:6),12.,0.,-1.)
         CALL PLCHMQ(CPUX(LOCX+72),CPUY(LOCY-40),'-',12.,0.,-1.)
...
C
 ...
 104        FORMAT(F6.1)
         ...
         IF (IOLAY.NE.2 .OR. LABFLG.GT.5) THEN
            CALL PLCHMQ(CPUX(LOCX+30),CPUY(LOCY-45),LABEL(1:6),12.,0.,
     X           -1.)
         END IF
         IF (IOLAY.GT.0) THEN
C
...
C
     ...
     X              J=2.5-SIGN(1.0,CLB(I,IFLD))
         ...
         END IF
         IF (LABFLG.GT.5) THEN
            CALL PLCHMQ(CPUX(LOCX+104),CPUY(LOCY-45),IPAT(J)(:1),8.,0.,
     X           -1.)
         END IF
         LOCY=LOCY-30
         CABDBZ(I)=RNULL
...
c         IF (IOLAY.EQ.2) LOCX=LOCX+35
         CALL PLCHMQ(CPUX(LOCX+25),CPUY(LOCY-40),LABEL(1:6),12.,0.,-1.)
         CALL PLCHMQ(CPUX(LOCX+99),CPUY(LOCY-40),IPAX,18.,0.,-1.)
