...
c----------------------------------------------------------------------X
c
      SUBROUTINE PLTVAD(NAMFLD,JVD,NVD,ZMNVD,ZMXVD,ISKPVD,WFILT,
     X     XMNVD,XMXVD,XSCLVD,XREFVD,TYPVD,NXVD,H0,KVD,NAMINF,NAMOUF,
     X     NAMDBZ,LABFLG)
C
...
C     NXVD    - Number of individual gridded levels allowed within a single
C               volume dataset (NXVD=MAXZLEV=51 set in CEDRIC.INC).
C     KVD     - Number of VAD analyses done in this volume scan.  Same
C               as the number of gridded vertical levels.
C     JVD     - Number of different VAD analyses to be plotted (1).
C     M       - LOOP INDEX FOR CURRENT VAD PLOTTING [NAMPLVD(M)]
C               This index will be =1 in Cedric since only one type
C               of VAD analysis has been done.
C     KVD     - Same as NZ, the number of vertical levels of the original 
C               gridded dataset.  Each vertical level is VAD-analyzed to
C               produce a vertical profile with NR heights (ranges).
...
C
     ...
     X                NCXORD(3),NFL,NPLANE,BAD
      ...
      DIMENSION TMP2(MXR,MXA)
      CHARACTER*6 IFMTX,IFMTY
      CHARACTER LABX*7,LABY*7,LABZ*6,LABL*100,LABH*3
      LOGICAL FRST

      PARAMETER (NVDMX=6)
      CHARACTER*8 NAMFLD(NVDMX)
      CHARACTER*4 TYPVD(NVDMX,NXVD)
      DIMENSION XMNVD(NVDMX,NXVD),XMXVD(NVDMX,NXVD)
      DIMENSION XSCLVD(NVDMX,NXVD),XREFVD(NVDMX,NXVD)
      DIMENSION ZMNVD(NXVD),ZMXVD(NXVD),ISKPVD(NXVD)
      DIMENSION WFILT(NXVD)
...
C     Move plots left (decrease x) by 0.05 from PPI values
C     Move plots down (decrease y) by 0.02 from PPI values
C
c      DATA XRT/0.32,0.54,0.76,0.98,0.32,0.54/
      DATA XRT/0.27,0.49,0.71,0.93,0.27,0.49/
c      DATA YTP/0.92,0.92,0.92,0.92,0.50,0.50/
      DATA YTP/0.90,0.90,0.90,0.90,0.48,0.48/
      DATA SIDEX,SIDEY/0.175,0.35/
c      DATA XRTU,YTPV,SIDE/0.98,0.50,0.35/
      DATA XRTU,YTPV,SIDE/0.93,0.48,0.35/
      DATA XOFF1,XOFF2,YOFF/0.0625,0.06,0.0425/
      DATA RE,TORAD/17000.0,0.017453293/

C     Parameters for frame labeling
C
      CHARACTER*80 LABEL
      CHARACTER*40 CITIT
      CHARACTER *8 NOWDAT
...
C     Loop over all VAD analyses for this volume of gridded data.
C     Each vertical level of the gridded data was used to create
C     a separate VAD analysis.  KVD is number of vertical levels
C     of the original gridded dataset.
C
 10   CONTINUE
      IVD=IVD+1

C     Print VAD wind profiles for debugging
C
C--------------------------
      NIN2='FOUR'
      FXOLD=VALLEV(IVD)
...
c      DO I=1,NR
      DO I=1,10
         ...
         IF(U0(I,IVD).NE.BDVAL)THEN
            WRITE(6,703)NIN2,I,SRNG,Z,U0(I,IVD),
     X           V0(I,IVD),SPD(I,IVD),DIR(I,IVD),-DIV(I,IVD),
     X           STR(I,IVD),SHR(I,IVD),ERR(I,IVD),DBZ(I,IVD)
 703        FORMAT(1X,A4,':    IRZ=',I4,2F8.3,'  UVSD=',4F8.2,
...
C--------------------------
      ...
      WRITE(LABL,21)VTYPE,RBEG,REND,RDEL,DXY,IVD
 21   FORMAT('METHOD: ',A8,'  Range: ',f4.1,' - ',f5.1,
     x     ' km  Rdel=',f4.2,' Dxy=',f4.2,' Level:',I3)
      ZLEV = VALLEV(IVD)
      J2=NINT(WFILT(M))
      J1=-J2
...
C
      DO 300 N=1,NVDMX
         ...
         Y2=YTP(N)
         Y1=YTP(N)-SIDEY
         CALL PPI_MAJMIN(XMNVD(N,M),XMXVD(N,M),IFMTX,MJRX,MNRX,IPLX)
         CALL PPI_MAJMIN(  ZMNVD(M),  ZMXVD(M),IFMTY,MJRY,MNRY,IPLY)
         IPLX=0
         IPLY=0
...
C        LOOP OVER ALL RANGES (HEIGHTS) WITHIN CURRENT VAD ANALYSIS
C
         KK=0
...
C     
      ...
      Y2=YTPV
      Y1=YTPV-SIDE
      CALL PPI_MAJMIN(XMNVD(1,M),XMXVD(1,M),IFMTX,MJRX,MNRX,IPLX)
      CALL PPI_MAJMIN(XMNVD(2,M),XMXVD(2,M),IFMTY,MJRY,MNRY,IPLY)
...
c     Plot a box to help positioning of VAD plots
c      CALL LINE(XL,YB,XR,YB)
c      CALL LINE(XR,YB,XR,YT)
c      CALL LINE(XR,YT,XL,YT)
c      CALL LINE(XL,YT,XL,YB)
      ...
      L2=NCWORD(2)
      L3=NCWORD(3)
      WRITE (LABEL,501)(ID(I),I=116,121),(ID(I),I=125,127),
     X                     (ID(I),I=13,15),AXNAM(L3),
     X                     ZLEV,LABAXS(L3,IUNAXS),NAMINVD(IVD)
 501  FORMAT(I2.2,'/',I2.2,'/',I2.2,6X,I2.2,2(':',I2.2),'-',I2.2,
     X     2(':',I2.2),7X,3A2,7X,A2,'=',F7.2,' ',A4,6X,A8)
      IF (LABFLG.GT.5) THEN
         CALL PLCHMQ(CPUX(60),CPUY(1010),LABEL,12.,0.,-1.)
         WRITE (LABEL,503)NOWDAT
 503     FORMAT('(AS OF ',A8,')')
         CALL PLCHMQ(CPUX(10),CPUY(985),LABEL(1:16),12.,0.,-1.)
         WRITE (LABEL,505)XREL,YREL,XOR
 505     FORMAT('ORIGIN=(',F7.2,',',F7.2,') KM   X-AXIS=',F5.1,' DEG')
         CALL PLCHMQ(CPUX(430),CPUY(985),LABEL(1:46),12.,0.,-1.)
         CALL PLCHMQ(CPUX(200),CPUY(960),CITIT,12.,0.,-1.)
         WRITE (LABEL,507)AXNAM(L2), LABAXS(L2,IUNAXS)
 507     FORMAT(A1,' ',A4)
      END IF
